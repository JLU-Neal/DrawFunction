...
public class SinView extends View {
    ...
    private Point mCoo = new Point(720, 630);//坐标系
    ...
    private void init() {
        ...
        mGridPicture = HelpDraw.getGrid(getContext(), 90);
    ...
    private void drawHelp(Canvas canvas) {
        canvas.drawText("100*sin(2x+π/2)+100", Df.last(), -f(Df.last()), mTextPaint);
        canvas.drawLine(-400,-100,400,-100,mHelpPint);
    }
...
//        float y= 100*(float) Math.pow(Math.E,x/300f);
        float y = (float) (100 * Math.sin(2*rad(x)+Math.PI/2)+100);
